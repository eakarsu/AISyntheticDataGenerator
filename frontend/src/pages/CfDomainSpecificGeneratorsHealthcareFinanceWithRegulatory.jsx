...
export default function CfDomainSpecificGeneratorsHealthcareFinanceWithRegulatory() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-domain-specific-generators-healthcare-finance-with-regulatory-presets/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-domain-specific-generators-healthcare-finance-with-regulatory-presets', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-domain-specific-generators-healthcare-finance-with-regulatory-presets/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Domain-specific generators (healthcare, finance) with regulatory presets</h1>
