...
export default function CfLlmPoweredSchemaInferenceAutoDetectingSchema() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-llm-powered-schema-inference-auto-detecting-schema-from-sample-data/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-llm-powered-schema-inference-auto-detecting-schema-from-sample-data', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-llm-powered-schema-inference-auto-detecting-schema-from-sample-data/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">LLM-powered schema inference auto-detecting schema from sample data</h1>
