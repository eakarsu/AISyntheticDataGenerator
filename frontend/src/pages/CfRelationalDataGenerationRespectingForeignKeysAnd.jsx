...
export default function CfRelationalDataGenerationRespectingForeignKeysAnd() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-relational-data-generation-respecting-foreign-keys-and-cardinality/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-relational-data-generation-respecting-foreign-keys-and-cardinality', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-relational-data-generation-respecting-foreign-keys-and-cardinality/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Relational data generation respecting foreign keys and cardinality</h1>
