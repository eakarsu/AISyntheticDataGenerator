...
export default function CfDistributionLearningCapturingStatisticalPropertiesFromReal() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-distribution-learning-capturing-statistical-properties-from-real-data/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-distribution-learning-capturing-statistical-properties-from-real-data', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-distribution-learning-capturing-statistical-properties-from-real-data/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Distribution learning capturing statistical properties from real data</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AISyntheticDataGenerator</p>
      </div>
