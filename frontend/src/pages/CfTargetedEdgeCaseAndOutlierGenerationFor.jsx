...
export default function CfTargetedEdgeCaseAndOutlierGenerationFor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-targeted-edge-case-and-outlier-generation-for-testing/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-targeted-edge-case-and-outlier-generation-for-testing', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-targeted-edge-case-and-outlier-generation-for-testing/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Targeted edge-case and outlier generation for testing</h1>
