...
export default function CfDifferentialPrivacySynthesisToPreventReIdentification() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-differential-privacy-synthesis-to-prevent-re-identification/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-differential-privacy-synthesis-to-prevent-re-identification', project: 'AISyntheticDataGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-differential-privacy-synthesis-to-prevent-re-identification/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Differential-privacy synthesis to prevent re-identification</h1>
