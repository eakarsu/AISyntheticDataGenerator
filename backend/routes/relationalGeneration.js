...
// Relational data generation respecting foreign keys and cardinality
// Feature: relational-generation
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISyntheticDataGenerator" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Relational data generation respecting foreign keys and cardinality.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'relational-generation', payload, result);
    res.json({ success: true, feature: 'relational-generation', ...result });
  } catch (err) {
    console.error('relational-generation analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISyntheticDataGenerator". Return strict JSON only.',
      `Feature: Relational data generation respecting foreign keys and cardinality. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'relational-generation', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'relational-generation', title: 'Relational data generation respecting foreign keys and cardinality', project: 'AISyntheticDataGenerator' });
});
